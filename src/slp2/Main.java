...
public class Main
{
  ...
  private Table interpStm(Stm.T prog,Table t)
  {
    ...
    if (prog instanceof Stm.Compound) {
      ...
      ExpList.T exp  = prnt.explist;
      while(exp != null)
      {
    	  IntAndTable iat = interpExplist(exp,t);
  ...
  public void doit(Stm.T prog)
  {
    ...
    if (Control.ConSlp.action == Control.ConSlp.T.COMPILE) {
      ...
      try {
        ...
        FileWriter writer = new FileWriter("slp2_gen.s");
        writer
            .write("// Automatically generated by the Tiger compiler, do NOT edit.\n\n");
        writer.write("\t.data\n");
        writer.write("slp2_format:\n");
        ...
        writer.write("\tleave\n\tret\n\n");
        writer.close();
        Process child = Runtime.getRuntime().exec("gcc slp2_gen.s");
        child.waitFor();
        if (!Control.ConSlp.keepasm)
          Runtime.getRuntime().exec("rm -rf slp2_gen.s");
      } catch (Exception e) {
        e.printStackTrace();
        System.exit(0);
      }
